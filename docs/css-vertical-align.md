...
![vertical-align](images/css-vertical-align/rc.png)
...
行盒子(line box)和内联级元素(inline-level elements)是非常重要的部分。[W3C](https://www.w3.org/TR/CSS21/visuren.html#inline-boxes "W3C")对其有专门的解释，总结起来可以这样理解：
...
基线是看不见的，但是我们可以通过添加一个小写字母"x"来判断它的位置。不同元素判断方法如下：

- *行盒子（line-box）:* 基线位于小写字母"x"的底部。
- *行内元素（inline）:* 同行盒子。
- *行内块元素（inline-block）:* 行内块元素的情况较多，需要分别说明。这边我们假设行盒子中有一个inline-block元素，并且设置它的margin-bottom值，我们在行盒子加上字母"x"方便找到行盒子的基线。
   - 无内容的行内块元素：基线位于元素的margin-bottom底部。
    
     ![](images/css-vertical-align/inline-block-1.png)
   
   - 有内容且overflow不为visible的行内块元素：基线与最后一个内容元素的基线重合。
    
     ![](images/css-vertical-align/inline-block-2.png)

   - 有内容且overflow为visible的行内块元素：基线位于元素的margin-bottom底部。
    
     ![](images/css-vertical-align/inline-block-3.png)
...
![](images/css-vertical-align/strut.png)
...
上面是[W3C](https://www.w3.org/TR/CSS21/visudet.html#propdef-vertical-align "W3C")对vetical-align的解释。由此可知vertical-align是专门为内联级元素在行盒子的垂直排列所设计的。

不同的取值产生的垂直排列效果是不一样的，下面我们分别来讲述。假设行盒子里面有3个行内块元素和一个行内元素。为了更快找到行盒子的基线我们添加了x字符，为了更好地理解“sub”和“super”我们添加了`<sub>`和`<sup>`。
...
**vertical-align**分别取值：
- **baseline（默认）**
![](images/css-vertical-align/baseline.png)

  行盒子基线和内联级元素基线对齐，这是默认值。
- **middle**
![](images/css-vertical-align/middle.png)

  内联级元素的垂直中线与行盒子的基线往上偏移半个x的位置对齐。

- **sub**
![](images/css-vertical-align/sub.png)

  内联级元素基线与`<sub>`基线重合。
  
- **super**
![](images/css-vertical-align/super.png)

  内联级元素基线与`<sup>`基线重合。
  
- **text-top**
![](images/css-vertical-align/text-top.png)

  内联级元素上边界和strut顶部对齐。**text-bottom**则是与底部对齐。

- **top**
![](images/css-vertical-align/top.png)

  内联级元素顶部和行盒子顶部对齐。**bottom**则是与底部对齐

- **`<length>`（eg:10px）**
![](images/css-vertical-align/length.png)
